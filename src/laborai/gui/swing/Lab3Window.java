...
public class Lab3Window extends JFrame implements ActionListener {
    ...
    private MapADTx<String, Zaidejas> map;
    private int sizeOfInitialSubSet, sizeOfGenSet, colWidth, initialCapacity;
    private float loadFactor;
    private final ZaidejuSukurimas zaidGamyba = new ZaidejuSukurimas();
    ...
    private void initComponents() {
        ...
        panButtons = new Panels(new String[]{
            ...
            MESSAGES.getString("button9")}, 1, NUMBER_OF_BUTTONS);
    ...
    public void mapGeneration(String filePath) {
        ...
        createMap();
        // Jei failas nenurodytas - generuojami zaidmobiliai ir talpinami atvaizdyje
        if (filePath == null) {
            Zaidejas[] zaidArray = zaidGamyba.ZaidejuPridavimas(sizeOfGenSet, sizeOfInitialSubSet);
            for (Zaidejas a : zaidArray) {
                map.put(
                        zaidGamyba.ZaidejaiAnalize(),//raktas
                        a);
            }
    ...
    public void mapAdd() throws MyException {
        Zaidejas a = zaidGamyba.SurinktiZaidejai();
        map.put(
                zaidGamyba.ZaidejaiAnalize(),// Raktas
                a);
        table.setModel(new TableModel(map.getModelList(panParam1.getTfOfTable().get(5).getText()),
    ...
    public void searchContainsValue(){

        String numeris;
        numeris = panParam3.getTfOfTable().get(0).getText();
        Zaidejas a = new Zaidejas();
        a = map.get(numeris);
        boolean i = map.containsValue(a);
        if(i)
            KsSwing.oun(taEvents, "Egzistuoja");
        else
            KsSwing.oun(taEvents, "Neegzistuoja");
    }

    public void putIfAbsent(){
        String numeris;
        numeris = panParam3.getTfOfTable().get(0).getText();
        Zaidejas a = new Zaidejas.Builder().buildRandom();
        Zaidejas target = map.putIfAbsent(numeris, a);
        if(target != null){
           KsSwing.oun(taEvents, "Sioje pozicijoje yra objecktas: " + target.toString());

        } else {
            KsSwing.oun(taEvents, "Si pozicija yra tuscia, ir i ja buvo irasyta: " + a.toString());
            table.setModel(new TableModel(map.getModelList(panParam1.getTfOfTable().get(5).getText()),
                panParam1.getTfOfTable().get(5).getText(), map.getMaxChainSize()), colWidth);
           updateHashtableParameters(true);
        }
    ...
    public void KeySet(){
        ...
        KsSwing.oun(taEvents, "Raktu sarasas: ");
        for(String key : set){
            KsSwing.oun(taEvents, key);
        }
    }



    public void numberOfEmpties() {KsSwing.oun(taEvents, "Tusciu elementu skaicius: " + map.numberOfEmpties()); }
    ...
    private void createMap() {
        switch (cmbCollisionTypes.getSelectedIndex()) {
            case 0:
                map = new MapKTUx<>(new String(), new Zaidejas(), initialCapacity, loadFactor, ht);
                break;
            case 1:
                map = new MapKTUOAv2<>(new String(), new Zaidejas(), initialCapacity, loadFactor, ht);
                break;
            // ...
